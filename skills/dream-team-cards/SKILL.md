...
- Draft inicial: camino real `openInitialPacks` (`src/data/cardsRepo.js` +
  `draftSquad` de `src/packOpening/draftSquad.js`): 25 cartas de la liga del DT
  (5 sobres × 5, filtra `cards.league_id`), mínimos 2 POR / 6 DEF / 6 MED /
  4 DEL y el resto ponderado por banda de rareza; se guardan en `user_cards`
  antes de la animación. `sobresIniciales(rng)` (3 × 5, garantiza
  1 POR / 4 DEF / 3 MED / 3 DEL) queda solo como **fallback offline**.
