...
const managerAge = document.getElementById('manager-age');
const managerCountry = document.getElementById('manager-country');
const managerRegion = document.getElementById('manager-region');
...
function crearJuego() {
  ...
  // MatchScene y TestScene no las arranca el flujo normal del juego
  // todavía, pero las registramos igual para que el panel de dev
  // (ver src/dev/) pueda saltar a ellas.
  game.scene.add('MatchScene', MatchScene);
...
btnCreateManager.addEventListener('click', async () => {
  ...
  const edad = Number(managerAge.value);
  const pais = managerCountry.value.trim();
  const region = managerRegion.value;

  if (!nombre || !pais || !region || !Number.isInteger(edad) || edad < 18 || edad > 99) {
    managerError.textContent = 'Completá nombre, país, región y una edad válida (18 a 99).';
    return;
  }
  ...
  try {
    const manager = await createManager({ userId: usuario.id, name: nombre, age: edad, country: pais, region });
