...
function cartaCruda(rng, rareza, pos) {
  ...
  const prueba = rng.pick(FOTOS_PRUEBA);
  return {
    id,
...
function abrirSobre(rng, { cartas, bonus = 0, garantizarPuestos = null }) {
  const out = [];
  for (let i = 0; i < cartas; i++) {
    out.push(cartaCruda(rng, sortearRareza(rng, bonus), garantizarPuestos?.[i] || null));
  }
  return out;
}
...
export function sobreRefuerzo(rng, posicionFinal) {
