...
- `ESCUDOTECA` (`src/data/escudoteca.js`) — único lugar de escudos reales: mapa nombre → URL (Escudoteca Paladar Negro + api-sports.io para Atlético/Girona/Mallorca/Real Oviedo). Incluye alias de nombre (variantes, nombres EA de FC24) y lookup normalizado `escudoDeNombre(nombre)`. Si un escudo no existe o falla el CDN, el sistema de fallback automático genera SVG.
